...
def detect_r_peaks(ecg_signal: np.ndarray, fs: int = 250,
    ...
    peaks, _ = find_peaks(
        np.abs(ecg_signal), height=threshold, distance=distance)
    return peaks


def calculate_t_qrs_ratio(ecg_signal: np.ndarray,
                          r_peaks: np.ndarray, fs: int = 250):
    """
    ...
    return {'t_qrs_ratio': mean_ratio, 'hypoxia_risk': 'high' if mean_ratio is not None and mean_ratio >
            0.25 else 'low' if mean_ratio is not None else 'unknown', 'num_beats': int(len(r_peaks))}
