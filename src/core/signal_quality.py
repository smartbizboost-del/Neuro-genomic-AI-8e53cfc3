...
def extract_sqi_features(ecg_channel, fs=250):
    ...
    features['pband2'] = np.trapz(psd[(freqs>=10) & (freqs<=20)], freqs[(freqs>=10) & (freqs<=20)])

    # pband4: power in 48-52 Hz (power line interference)
    features['pband4'] = np.trapz(psd[(freqs>=48) & (freqs<=52)], freqs[(freqs>=48) & (freqs<=52)])
    ...
    features['snr'] = 10 * np.log10(np.var(ecg_channel) / np.var(np.diff(ecg_channel)))
