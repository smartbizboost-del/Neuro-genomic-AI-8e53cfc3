...
def process_ecg_file(self,
                     file_id: str,
                     s3_key: str,
                     gestational_weeks: int = None,
                     patient_id: str = None) -> Dict[str,
                                                     Any]:
    """
    ...
    try:
        ...
        self.update_state(
            state="PROCESSING", meta={
                "status": "Computing HRV features via Blind Source Separation"})
...
def extract_raw_signals(file_path: str) -> np.ndarray:
    ...
    if wfdb is not None:
        try:
            ...
            return np.asarray(record.p_signal, dtype=float)
        except Exception:
            pass
    ...
    # so the pipeline BSS functions don't crash from parsing errors during
    # basic demonstration bounds.
    ...
    return np.column_stack(
        [mixed, mixed * 0.8 + np.random.normal(0, 0.1, len(mixed))])
