...
# API: python -m uvicorn src.api.main:app --reload --port 8000
...
To build images and deploy to a Kubernetes cluster automatically, add a repository secret named `KUBE_CONFIG` containing your base64-encoded kubeconfig, then push to `main`. The workflow `deploy-k8s.yml` will build images, push to GHCR, and apply the manifests under `infrastructure/kubernetes`.

Ensure the cluster namespace `neuro-genomic` exists and required secrets (e.g., `neuro-secrets`) are configured in-cluster.
